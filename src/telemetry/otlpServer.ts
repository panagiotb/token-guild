...
function parseSpan(span: unknown, fallback: string): ParsedOtlpEvent | undefined {
  ...
  const outputTokens = numericAttribute(values, 'gen_ai.usage.output_tokens', 'gen_ai.usage.completion_tokens');
...
export class OtlpTelemetryServer {
  ...
  private async handle(request: http.IncomingMessage, response: http.ServerResponse): Promise<void> {
    ...
    if (request.method !== 'POST' || request.url !== DEFAULT_OTLP_PATH) {
      response.writeHead(404, { 'Content-Type': 'application/json' }); response.end('{"error":"Not found"}'); request.resume(); return;
    }
    if (!jsonContentType(request.headers['content-type']) || (request.headers['content-encoding'] && request.headers['content-encoding'] !== 'identity')) {
      response.writeHead(415, { 'Content-Type': 'application/json' }); response.end('{"error":"JSON without content encoding required"}'); request.resume(); return;
    }
    ...
    try { parsed = extractOtlpEvents(payload, this.maxSpans); }
    catch { response.writeHead(400, { 'Content-Type': 'application/json' }); response.end('{"error":"Invalid OTLP trace payload"}'); return; }
    let accepted = 0;
