...
describe('MVP registry', () => {
  it('loads the frozen content and resolves references', () => {
    const registry = loadMvpRegistry({ classes, weapons, passives, stages });
  ...
  it('rejects duplicate or broken registry references', () => {
    expect(() => loadMvpRegistry({ classes: [{ ...classes[0], id: classes[1]!.id }, ...classes.slice(1)], weapons, passives, stages })).toThrow(/duplicate IDs/);
    expect(() => loadMvpRegistry({ classes: [{ ...classes[0], startingWeaponId: 'missing' }, ...classes.slice(1)], weapons, passives, stages })).toThrow(/missing weapon/);
  });
});
