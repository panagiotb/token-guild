# Passive accessories and Guild Hall PowerUps: planning map

> Planning status (2026-08-01): retained reference, not a shipped contract. Several registered PowerUps are not yet reachable or behaviorally complete in production; verify against the current implementation.

## 1. Overview & Mechanics

In **Token Guild**, passive items and power-up meta-upgrades provide permanent and per-run bonuses to player stats.

* **In-Run Passive Accessories:** Found during dungeon runs via level-up selection (max 6 slots) or floor drops. They enhance combat metrics, boost hero survivability, and enable weapon evolutions.
* **Guild Hall PowerUp proposals:** The state layer contains several bounded upgrade/refund foundations, but only Guild Might is currently exposed in the webview. Wider purchasing and refund behavior remains planned.

---

## 2. Exhaustive In-Run Passive Accessories (1:1 Reskin Matrix)

| Passive Item | VS Equivalent | Max Level | Per-Level Bonus | Max Stat Bonus | Weapon Evolution Synergy | Token Guild Reskin Name |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Power Gauntlets** | Spinach | 5 | $+10\%$ Might | $+50\%$ Might | Dragon Breath | **Power Gauntlets** |
| **Iron Armor** | Armor | 5 | $+1$ Armor, $-1$ Damage Taken | $+5$ Armor | Bouncing Arrow *(Runetracer)* | **Iron Armor** |
| **Heart of Vitality** | Hollow Heart | 5 | $+20\%$ Max Health | $+100\%$ Max HP | Broadsword *(Whip)* | **Heart of Vitality** |
| **Phoenix Amulet** | Pummarola | 5 | $+0.2$ HP/sec Recovery | $+1.0$ HP/sec | Aegis Barrier *(Garlic)* | **Phoenix Amulet** |
| **Haste Amulet** | Empty Tome | 5 | $-8\%$ Cooldown | $-40\%$ Cooldown | Arcane Bolt *(Magic Wand)* | **Haste Amulet** |
| **Orb of Expansion** | Candelabrador | 5 | $+10\%$ Attack Area | $+50\%$ Area | Battle Axe *(Axe)* | **Orb of Expansion** |
| **Iron Bracer** | Bracer | 5 | $+10\%$ Projectile Speed | $+50\%$ Speed | Throwing Daggers *(Knife)* | **Iron Bracer** |
| **Spellbinder Scroll** | Spellbinder | 5 | $+10\%$ Attack Duration | $+50\%$ Duration | Orbiting Grimoire *(King Bible)* | **Spellbinder Scroll** |
| **Ring of Duplication** | Duplicator | 2 | $+1$ Projectile Amount | $+2$ Amount | Chain Lightning | **Ring of Duplication** |
| **Wings of Hermes** | Wings | 5 | $+10\%$ Movement Speed | $+50\%$ MoveSpeed | Void Dart *(Shadow Pinion)* | **Wings of Hermes** |
| **Token Magnetism** | Attractorb | 5 | $+33\%$ Pickup Radius | $+165\%$ Magnet | Alchemist Fire *(Santa Water)* | **Token Magnetism** |
| **Clover of Fortune** | Clover | 5 | $+10\%$ Luck | $+50\%$ Luck | Celestial Cross *(Cross)* | **Clover of Fortune** |
| **Crown of Wisdom** | Crown | 5 | $+8\%$ Growth (XP Gain) | $+40\%$ Growth | Purifying Nova *(Pentagram)* | **Crown of Wisdom** |
| **Stone Mask** | Stone Mask | 5 | $+10\%$ Greed (Gold Gain) | $+50\%$ Greed | Feline Spirit *(Gatti Amari)* | **Stone Mask** |
| **Skull of Doom** | Skull O'Maniac | 5 | $+10\%$ Curse (Enemy Stats) | $+50\%$ Curse | Harmonic Wave *(Song of Mana)* | **Skull of Doom** |
| **Tiragisú Ankh** | Tiragisú | 2 | $+1$ Revival (50% HP) | $+2$ Revivals | Twin Flintlocks *(Phieraggi)* | **Tiragisú Ankh** |
| **Pandora's Box** | Torrona's Box | 9 | $+4\%$ All Stats (L1-8), $+100\%$ Curse (L9) | $+32\%$ All, $+100\%$ Curse | Victory Sword $\rightarrow$ Sole Solution | **Pandora's Box** |
| **Aegis Left Crest**| Metaglio Left | 9 | $+0.1$ Recovery, $+5\%$ Max HP | $+0.9$ HP/s, $+45\%$ HP | Holy Shield $\rightarrow$ Crimson Shroud | **Aegis Left Crest** |
| **Aegis Right Crest**| Metaglio Right | 9 | $+5\%$ Curse | $+45\%$ Curse | Holy Shield $\rightarrow$ Crimson Shroud | **Aegis Right Crest** |
| **Ring of Wealth** | Gold Ring | 9 | $+5\%$ Curse, $+5\%$ Greed | $+45\%$ Curse, $+45\%$ Greed | Chrono Ray $\rightarrow$ Infinite Corridor | **Ring of Wealth** |
| **Ring of Clarity** | Silver Ring | 9 | $+5\%$ Duration, $+5\%$ Area | $+45\%$ Duration, $+45\%$ Area | Chrono Ray $\rightarrow$ Infinite Corridor | **Ring of Clarity** |

---

## 3. Explicit Level-by-Level Stat Progression Tables

### 1. **Power Gauntlets** *(VS: Spinach — 5 Levels)*
* **Level 1:** $+10\%$ Might (Damage).
* **Level 2:** $+20\%$ Might total (+10%).
* **Level 3:** $+30\%$ Might total (+10%).
* **Level 4:** $+40\%$ Might total (+10%).
* **Level 5:** $+50\%$ Might total (+10%).

---

### 2. **Iron Armor** *(VS: Armor — 5 Levels)*
* **Level 1:** $+1$ Armor (Reduces incoming damage by 1 per hit).
* **Level 2:** $+2$ Armor total (+1).
* **Level 3:** $+3$ Armor total (+1).
* **Level 4:** $+4$ Armor total (+1).
* **Level 5:** $+5$ Armor total (+1).

---

### 3. **Heart of Vitality** *(VS: Hollow Heart — 5 Levels)*
* **Level 1:** $+20\%$ Max HP.
* **Level 2:** $+40\%$ Max HP total (+20%).
* **Level 3:** $+60\%$ Max HP total (+20%).
* **Level 4:** $+80\%$ Max HP total (+20%).
* **Level 5:** $+100\%$ Max HP total (+20%).

---

### 4. **Phoenix Amulet** *(VS: Pummarola — 5 Levels)*
* **Level 1:** $+0.2$ HP/sec Recovery.
* **Level 2:** $+0.4$ HP/sec Recovery (+0.2).
* **Level 3:** $+0.6$ HP/sec Recovery (+0.2).
* **Level 4:** $+0.8$ HP/sec Recovery (+0.2).
* **Level 5:** $+1.0$ HP/sec Recovery (+0.2).

---

### 5. **Haste Amulet** *(VS: Empty Tome — 5 Levels)*
* **Level 1:** $-8\%$ Cooldown.
* **Level 2:** $-16\%$ Cooldown (-8%).
* **Level 3:** $-24\%$ Cooldown (-8%).
* **Level 4:** $-32\%$ Cooldown (-8%).
* **Level 5:** $-40\%$ Cooldown (-8%).

---

### 6. **Orb of Expansion** *(VS: Candelabrador — 5 Levels)*
* **Level 1:** $+10\%$ Attack Area.
* **Level 2:** $+20\%$ Attack Area (+10%).
* **Level 3:** $+30\%$ Attack Area (+10%).
* **Level 4:** $+40\%$ Attack Area (+10%).
* **Level 5:** $+50\%$ Attack Area (+10%).

---

### 7. **Iron Bracer** *(VS: Bracer — 5 Levels)*
* **Level 1:** $+10\%$ Projectile Speed.
* **Level 2:** $+20\%$ Projectile Speed (+10%).
* **Level 3:** $+30\%$ Projectile Speed (+10%).
* **Level 4:** $+40\%$ Projectile Speed (+10%).
* **Level 5:** $+50\%$ Projectile Speed (+10%).

---

### 8. **Spellbinder Scroll** *(VS: Spellbinder — 5 Levels)*
* **Level 1:** $+10\%$ Effect Duration.
* **Level 2:** $+20\%$ Effect Duration (+10%).
* **Level 3:** $+30\%$ Effect Duration (+10%).
* **Level 4:** $+40\%$ Effect Duration (+10%).
* **Level 5:** $+50\%$ Effect Duration (+10%).

---

### 9. **Ring of Duplication** *(VS: Duplicator — 2 Levels)*
* **Level 1:** $+1$ Projectile Amount to all active weapons.
* **Level 2:** $+2$ Projectile Amount total (+1).

---

### 10. **Wings of Hermes** *(VS: Wings — 5 Levels)*
* **Level 1:** $+10\%$ Movement Speed.
* **Level 2:** $+20\%$ Movement Speed (+10%).
* **Level 3:** $+30\%$ Movement Speed (+10%).
* **Level 4:** $+40\%$ Movement Speed (+10%).
* **Level 5:** $+50\%$ Movement Speed (+10%).

---

### 11. **Token Magnetism** *(VS: Attractorb — 5 Levels)*
* **Level 1:** $+33\%$ Pickup Radius.
* **Level 2:** $+66\%$ Pickup Radius (+33%).
* **Level 3:** $+99\%$ Pickup Radius (+33%).
* **Level 4:** $+132\%$ Pickup Radius (+33%).
* **Level 5:** $+165\%$ Pickup Radius (+33%).

---

### 12. **Clover of Fortune** *(VS: Clover — 5 Levels)*
* **Level 1:** $+10\%$ Luck.
* **Level 2:** $+20\%$ Luck (+10%).
* **Level 3:** $+30\%$ Luck (+10%).
* **Level 4:** $+40\%$ Luck (+10%).
* **Level 5:** $+50\%$ Luck (+10%).

---

### 13. **Crown of Wisdom** *(VS: Crown — 5 Levels)*
* **Level 1:** $+8\%$ XP Growth.
* **Level 2:** $+16\%$ XP Growth (+8%).
* **Level 3:** $+24\%$ XP Growth (+8%).
* **Level 4:** $+32\%$ XP Growth (+8%).
* **Level 5:** $+40\%$ XP Growth (+8%).

---

### 14. **Stone Mask** *(VS: Stone Mask — 5 Levels)*
* **Level 1:** $+10\%$ Gold Greed.
* **Level 2:** $+20\%$ Gold Greed (+10%).
* **Level 3:** $+30\%$ Gold Greed (+10%).
* **Level 4:** $+40\%$ Gold Greed (+10%).
* **Level 5:** $+50\%$ Gold Greed (+10%).

---

### 15. **Skull of Doom** *(VS: Skull O'Maniac — 5 Levels)*
* **Level 1:** $+10\%$ Enemy Curse (Speed, Health, Quantity).
* **Level 2:** $+20\%$ Enemy Curse (+10%).
* **Level 3:** $+30\%$ Enemy Curse (+10%).
* **Level 4:** $+40\%$ Enemy Curse (+10%).
* **Level 5:** $+50\%$ Enemy Curse (+10%).

---

### 16. **Tiragisú Ankh** *(VS: Tiragisú — 2 Levels)*
* **Level 1:** $+1$ Life Revival (revives with 50% HP).
* **Level 2:** $+2$ Life Revivals total (+1).

---

### 17. **Pandora's Box** *(VS: Torrona's Box — 9 Levels)*
* **Level 1:** $+4\%$ Might, Speed, Area, Duration.
* **Level 2:** $+8\%$ Might, Speed, Area, Duration (+4%).
* **Level 3:** $+12\%$ Might, Speed, Area, Duration (+4%).
* **Level 4:** $+16\%$ Might, Speed, Area, Duration (+4%).
* **Level 5:** $+20\%$ Might, Speed, Area, Duration (+4%).
* **Level 6:** $+24\%$ Might, Speed, Area, Duration (+4%).
* **Level 7:** $+28\%$ Might, Speed, Area, Duration (+4%).
* **Level 8:** $+32\%$ Might, Speed, Area, Duration (+4%).
* **Level 9:** $+100\%$ Enemy Curse (High risk / required for Sole Solution evolution).

---

## 4. Guild Hall Meta-Powerups (1:1 Reskin Matrix)

Purchased in the Guild Hall meta-shop to upgrade permanent account stats across all runs.

| Guild PowerUp | VS Equivalent | Max Rank | Stat Increase per Rank | Base Gold Cost | Cost Scaling Multiplier | Token Guild Description |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Guild Might** | Might | 5 | $+5\%$ Damage | 200 Gold | $1.10\times$ | Increases raw damage output of all hero weapons. |
| **Guild Armor** | Armor | 3 | $+1$ Armor | 600 Gold | $1.15\times$ | Reduces damage taken from enemy mob collisions. |
| **Guild Vitality** | Max Health | 3 | $+10\%$ Max Health | 200 Gold | $1.10\times$ | Increases maximum HP bar capacity. |
| **Guild Recovery** | Recovery | 5 | $+0.1$ HP/sec | 200 Gold | $1.10\times$ | Passive health regeneration per second. |
| **Guild Haste** | Cooldown | 2 | $-2.5\%$ Cooldown | 900 Gold | $1.20\times$ | Increases weapon firing rate across all attacks. |
| **Guild Expansion**| Area | 2 | $+5\%$ Attack Area | 300 Gold | $1.10\times$ | Expands projectile and aura hitboxes. |
| **Guild Swiftness**| Speed | 2 | $+10\%$ Projectile Speed | 300 Gold | $1.10\times$ | Speeds up traveling projectiles. |
| **Guild Duration** | Duration | 2 | $+10\%$ Effect Duration | 300 Gold | $1.10\times$ | Extends lifetime of orbiting and floor spells. |
| **Guild Duplication**| Amount | 1 | $+1$ Extra Projectile | 5,000 Gold | $1.50\times$ | Adds +1 projectile to all active weapons. |
| **Guild Agility** | Move Speed | 2 | $+10\%$ Movement Speed | 300 Gold | $1.10\times$ | Increases hero walking velocity in dungeon canvas. |
| **Token Magnetism** | Magnet | 2 | $+25\%$ Pickup Radius | 300 Gold | $1.10\times$ | Widens gem and coin auto-attraction distance. |
| **Guild Fortune** | Luck | 3 | $+10\%$ Luck | 600 Gold | $1.15\times$ | Increases chances for 3-card and 5-card chest jackpots. |
| **Token Growth** | Growth | 5 | $+3\%$ XP per stream token | 300 Gold | $1.10\times$ | Increases XP earned per streamed token. |
| **Gold Hoard** | Greed | 5 | $+10\%$ Gold drop rate | 200 Gold | $1.10\times$ | Increases gold coin rewards from boss chests. |
| **Chaos Curse** | Curse | 5 | $+10\%$ Enemy Speed & Health| 1,666 Gold | $1.25\times$ | Increases enemy spawn density for faster leveling. |
| **Ankh Revival** | Revival | 1 | $+1$ Life Revival | 10,000 Gold | $2.00\times$ | Revives hero with 50% HP upon death. |
| **Guild Reroll** | Reroll | 5 | $+1$ Level-up Reroll | 1,000 Gold | $1.20\times$ | Rerolls 3-card level-up upgrade choices. |
| **Guild Skip** | Skip | 5 | $+1$ Level-up Skip | 500 Gold | $1.15\times$ | Skips level-up choice and awards bonus XP. |
| **Guild Banish** | Banish | 5 | $+1$ Item Banish | 1,000 Gold | $1.20\times$ | Removes selected item from current run drop pool. |
